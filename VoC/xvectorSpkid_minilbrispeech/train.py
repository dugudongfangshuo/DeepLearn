...
class SpkIdBrain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        return predictions
    # 从dataset里计算特征，这里的特征计算是在训练的时候，每个batch进入模型前计算一次
    def prepare_features(self, wavs, stage):
        ...
        if stage == sb.Stage.TRAIN:
            if hasattr(self.modules, "env_corrupt"):
                ...
                # 加上了环境噪音之后一并放入了batch，将batch扩大了2倍
    ...
    # 每一个echo开始训练之前，初始化一下metric记录信息的对象
...
def dataio_prep(hparams):
    ...
    # 注意这里只是把数据读进来了，dynamic_items还没有处理数据，他在dataloader的时候才会根据训练迭代来处理数据
    ...
    # 更新了一下label_encoder，然后后面就可以进行编码了
