...
class Xvector(torch.nn.Module):
    ...
    def forward(self, x, lens=None):
        ...
        for layer in self.blocks:
            try:
                # 这里会自动跳过，通过报错进入到except，继续执行程序，而不会停止
                x = layer(x, lengths=lens)
            except TypeError:
                x = layer(x)
        return x

# 这个类Sequential继承了ModuleDict，不同于官方实现是继承了module，并且实现了一个append方法，
# 所以调用的格式是输入字典形式的layer,而且这个containers.Sequential实现了forward，不用再实现一遍了
# append(layer函数名，layer的一些参数，layer名)
# Sequential__init__的时候要输入一下input的shape，再插入的layer的inputshape都是这个了就，所以这有个问题
# 就是要注意循环添加一些层的时候输入输出的channel要相同
class Classifier(sb.nnet.containers.Sequential):
    ...
    def __init__(
        ...
        if lin_blocks > 0:
            # 再次插入一个Sequential.嵌套了一个模块就相当，因为都继承同一个类module.所以完全没问题
            self.append(sb.nnet.containers.Sequential, layer_name="DNN")

        # Adding fully-connected layers
        for block_index in range(lin_blocks):
            block_name = f"block_{block_index}"
            # 使用了modulelist,给层命名了就可以直接使用名字来调用这个层了
            self.DNN.append(
